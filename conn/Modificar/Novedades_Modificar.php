...
$Nov_ID=isset($_REQUEST['Nov_ID'])? $_REQUEST['Nov_ID']:null;
...
if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    $miUpdate = $miPDO->prepare("UPDATE novedades SET Nov_Descripcion='$Nov_Descripcion',Nov_Tipo='$Nov_Tipo',Nov_Fecha='$Nov_Fecha'  WHERE Nov_ID='$Nov_ID';");
    //ejecutar update 
    $miUpdate->execute();
    //redireccionar a leer
    header('Location: ../../Administrador/4_Novedades_Administrador.php');

}else{
    //preparar select
    $miConsulta=$miPDO->prepare("SELECT * FROM novedades WHERE Nov_ID='$Nov_ID';");
    //ejecutar consulta
    $miConsulta->execute();
...
<form method="POST">
        ...
        <input type="text" name="Prd_ID" id="" class="form-control" value="<?=$novedades['Prd_ID']?>" disabled>
        <br><br>
        <label for="">Cliente</label><br>
        <input type="text" name="Cli_ID" id="" class="form-control" value="<?=$novedades['Cli_ID']?>" disabled>
        <br><br>
        <label for="">Tipo de novedad</label><br>
        <select name="Nov_Tipo" id="" class="form-control">
            <option value="<?=$novedades['Nov_Tipo']?>">-Seleecionar tipo de novedad-</option>
            <option value="Garantia">Garantia</option>
            <option value="Devolución">Devolución</option>
        </select>
         <br><br>
         <label for="">Descripción de novedad</label><br>
        <input type="text" name="Nov_Descripcion" id="" class="form-control" value="<?=$novedades['Nov_Descripcion']?>"> 
         <br><br>
         <label for="">Fecha de novedad</label><br>
        <input type="date" name="Nov_Fecha" id="" class="form-control" value="<?=$novedades['Nov_Fecha']?>">
         <br><br>
         <input type="submit" value="Modificar" class="btn btn-warning">  <a href="../../Administrador/4_Novedades_Administrador.php"><button type="button" class="btn btn-danger" data-bs-dismiss="modal">Cancelar</button></a>
