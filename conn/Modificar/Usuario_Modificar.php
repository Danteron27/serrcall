...
//RECOJE LAS VARIABLES
    ...
    $Usu_Login=isset ($_REQUEST['login']) ? $_REQUEST['login']:null ;
    $Usu_Password=isset($_REQUEST['pass']) ? $_REQUEST['pass']:null ;
    $Usu_Rol=isset($_REQUEST['rol']) ? $_REQUEST['rol']:null ;
// Conexion con la BD
...
if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    $miUpdate = $miPDO->prepare("UPDATE usuarios SET Usu_Login='$Usu_Login', Usu_Password='$Usu_Password',Usu_Rol='$Usu_Rol' WHERE Usu_ID='$Usu_ID';");
    //ejecutar update 
    $miUpdate->execute();
    ...
    //preparar select
    $miConsulta=$miPDO->prepare("SELECT * FROM usuarios WHERE Usu_ID='$Usu_ID';");
    //ejecutar consulta
    $miConsulta->execute();
...
<form action="" method="POST">
        ...
        <input type="text" name="Usu_ID" id="" class="form-control" disabled value="<?=$usuarios['Usu_ID']?>">
        <br><br>
        <label for="">Usuario</label><br>
        <input type="text" name="login" id="" class="form-control" value="<?=$usuarios['Usu_Login']?>">
        <br><br>
        <label for="">Contraseña</label><br>
        <input type="text" name="pass" id="" class="form-control" value="<?=$usuarios['Usu_Password']?>">
        <br><br>
        <label for="">Rol</label><br>
        <select name="rol" id="" class="form-control">
            <option value="<?=$usuarios['Usu_Rol']?>">-Seleccionar rol-</option>
            <option value="admin">Administrador</option>
            <option value="usuarios">Asesor de ventas</option>
        </select>
        <br>
        <input type="submit" value="Modificar" class="btn btn-warning">  <a href="../../Administrador/5_Usuario_Administrador.php"><button type="button" class="btn btn-danger" data-bs-dismiss="modal">Cancelar</button></a>
    </form>       
  </div>
</center>
